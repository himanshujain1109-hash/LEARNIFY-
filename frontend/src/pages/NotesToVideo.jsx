...
const VIDEO_API =
  (import.meta.env.VITE_VIDEO_API_URL || "http://localhost:8000").replace(
    /\/+$/,
    ""
  );
...
export default function NotesToVideo() {
  ...
  const generateVideo = async (event) => {
    ...
    if (file.size > MAX_VIDEO_MB * 1024 * 1024) {
      setStatus("error");
      setMessage(`This file is too large. The maximum is ${MAX_VIDEO_MB} MB.`);
      return;
    }
    ...
    <div className="notes-video-page">
      ...
      <div className="notes-video-grid">
        <form className="upload-card" onSubmit={generateVideo}>
          ...
          <p className="muted">
            Supported formats: PPTX, PDF and TXT. Runs locally; no Gemini/API key required.
          </p>
            ...
            <label htmlFor="video-file">
              ...
              <span>
                {file
                  ...
                  : `PPTX · PDF · TXT · max ${MAX_VIDEO_MB} MB`}
              </span>
            </label>
            <input
              id="video-file"
          ...
          <div className="video-options" style={{ marginTop: 20 }}>
            ...
            <div className="row" style={{ gap: 10, flexWrap: "wrap", marginTop: 12 }}>
              ...
              <label>Level
                <select value={level} onChange={e => setLevel(e.target.value)}>
                  <option>Beginner</option><option>CBSE</option><option>College</option><option>JEE</option>
                </select>
              </label>
              <label>Style
                <select value={style} onChange={e => setStyle(e.target.value)}>
                  <option>Teacher</option><option>Storytelling</option><option>Exam-focused</option><option>Simple explanation</option>
                </select>
              </label>
              <label>Duration
              ...
              <label>Voice
                <select value={voice} onChange={e => setVoice(e.target.value)}>
                  <option value="default">System default</option><option value="male">Male</option><option value="female">Female</option>
                </select>
              </label>
            </div>
          </div>
